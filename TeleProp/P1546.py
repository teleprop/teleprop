...
def bt_loss(f, t, heff, h2, R2, area, d_v, path_c, pathinfo, *args):
    ...
    if (debug == 1):
        if dtot < 1: 
            Edebug = E
            fid_log.write('Field strength for d < 1 km (dB),S15 (38),17,' + '%.8f,\n' %(Edebug))
        else:
            Edebug = []
    ...
    if (debug == 1):
        if (isempty(Edebug)):
            ...
            fid_log.write('Field strength for q <> 50 %,S12 (33),18,'+ '%.8f,\n' %(Edebug))
    ...
    if (debug==1):
        fid_log.write('Resulting field strength for Ptx = 1kW (dBuV/m), , ,' + '%.8f,\n' %(E))
    ...
    if (debug==1):
        fid_log.write('Resulting field strength for given PTx (dBuV/m), , ,' + '%.8f,\n' %(E))
        fid_log.write('Resulting basic transmission loss (dB), S17 (40),20,' + '%.8f,\n' %(L))
        if(inside_file==1):
            try:
                fid_log.close()
...
def step82(tabulatedValues, h1, dinf, dsup, d, path, fnom, f, Emaxvalue, t):
    ...
    if path.find('Land') != -1:
        if h1 >= 0:
            ...
            return E
        elif h1 < 0:   # STI: the following seems to be wrong
            # v = V(fnom,h1)
        ...
        if d <= Dh1:
            ...
            ED20 = E10D20 + (E20D20 - E10D20) * np.log10(h1 / 10.) / np.log10(20. / 10.)
            #EDh1 = emax(Dh1, t, 'Sea')
            ...
            E1 = E10 + (E20 - E10) * np.log10(h1 / 10.) / np.log(20. / 10.)
            v = V(fnom, -10.)
            if v > -0.7806:
                J = 6.9 + 20 * np.log10(np.sqrt((v - 0.1) ** 2 + 1) + v - 0.1)  # equ'n (12a)
            else:
...
def V(Kv, h1):
    ...
    if Kv == 100:
        ...
        deg = 6.0 * atand(-h1 / 9000.)       #equ'n (12c and 12b)
        return deg
...
def J(nu):
    ...
    -------------------------------------------------------------------------------
     ...
     v1    12AUG13     Ivica Stevanovic, OFCOM         Initial version
    """
    return   6.9 + 20. * np.log10(np.sqrt((nu - 0.1) ** 2.0 + 1.0) + nu - 0.1)    
...
def step_13a(d, f, t, eff1, eff2):
    ...
    """
    thetaS = 180.0 * d / np.pi / 4. * 3. / 6370. + eff1 + eff2    #%equ'n (35)
    if thetaS < 0:
        thetaS = 0.0
...
def plotTeff1(ax,d,h,hT,teff1):
    ...
    y1=h[0]+hT
    y2=h[0]+hT+(x2-x1)*1000*np.tan(teff1*np.pi/180.)
...
def   clutter(i, ClutterCodeType):
    ...
    if strcmp(ClutterCodeType,'OFCOM'):
        ...
        elif i==1:	RxClutterCode='Water (salt)'
        elif i==2:	RxClutterCode='Water (fresh)'
        elif i==3:	RxClutterCode='Road/Freeway'
        elif i==4:	RxClutterCode='Bare land'
        elif i==5:	RxClutterCode='Bare land/rock'
        elif i==6:	RxClutterCode='Cultivated land'
        elif i==7:	RxClutterCode='Scrub'
        elif i==8:	RxClutterCode='Forest'
        elif i==9:	RxClutterCode='Low dens. suburban'
        elif i==10: RxClutterCode='Suburban'
        elif i==11: RxClutterCode='Low dens. urban'
        elif i==12: RxClutterCode='Urban'
        elif i==13: RxClutterCode='Dens. urban'
        elif i==14: RxClutterCode='High dens. urban'
        elif i==15: RxClutterCode='High rise industry'
        elif i==16: RxClutterCode='Skyscraper'
